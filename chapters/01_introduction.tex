...
Similar to our application case, \cite{sugar_beet_mechanical_damage} also detected damage of sugar beet plants. The difference to our goal was to predict mechanical damages caused by harvesting the plants. The camera used was located directly inside the harvester machine. The author compared different architectures including YOLOv4 (\cite{yolov4}), region-based fully convolutional network (R-FCN) and faster regions with convolutional neural network features (Faster R-CNN). The results of their work show that YOLOv4 predicted the sugar beets with better accuracy and also faster than the other two methods. \\
